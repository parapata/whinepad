...
jest.dontMock("../components/Dialog").dontMock("../components/Button");
...
describe("renders with action buttons", () => {
  it("can haz Cancel", () => {
    ...
    const cancel = TestUtils.findRenderedDOMComponentWithClass(
      dialog,
      "DialogDismiss"
    );
  ...
  it("can haz a single dismiss button", () => {
    const dialog = TestUtils.renderIntoDocument(
      <Dialog hasCancel={false} confirmLabel="confirm">
        Civilized dialog
      </Dialog>
    );
    const cancels = TestUtils.scryRenderedDOMComponentsWithClass(
      dialog,
      "DialogDismiss"
    );
    expect(cancels.length).toBe(0);
    let ok = TestUtils.findRenderedDOMComponentWithTag(dialog, "button");
  ...
  it("has head and body", () => {
    ...
    expect(node.getElementsByClassName("DialogHeader")[0].innerHTML).toBe(
      "head"
    );
    expect(node.querySelector(".DialogBody").textContent).toBe(
      "Civilized dialog"
    );
  });
